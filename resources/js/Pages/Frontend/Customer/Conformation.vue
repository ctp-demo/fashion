...
<template>
    <Layout>
        <div class="row my-5">
            <div class="col-12 col-lg-8 text-center mb-3 mx-auto">
                <h4 class="mt-2">Thank You! 😇</h4>
                <p>For you ordering. we will response you as soon as possible for conform your order.</p>
                <a href="/dashboard" class="btn round btn-outline-primary">My Dashboard</a>
            </div>
        </div>
        <div>
            <section class="app-user-list" v-if="props.order">
                <div class="row">
                                ...
                                <div class="d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-0">
                                    <div>
                                        <div class="logo-wrapper">
                                            <img :src="`/storage/${dataStore.settingItem.find(item => item.key === 'header_logo')?.val}`" style="max-width: 65px" alt="">
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tr v-for="(item, index) in  order.order_details">
                                        ...
                                        <td class="py-1">
                                            <span class="fw-bold" v-if="item.product?.discount > 0">{{ item.product?.discount }}</span>
                                            <span class="fw-bold" v-else>{{ item.product?.price }}</span>
                                        </td>
                                        <td class="py-1">
                                        ...
                                        <td class="py-1">
                                            ...
                                            <span class="fw-bold" v-if="item.product?.discount > 0">{{ (item.product?.discount) *  item?.quantity}}</span>
                                            <span class="fw-bold" v-else>{{ (item.product?.price) *  item?.quantity}}</span>
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
                                    ...
                                    <div class="col-md-4 d-flex justify-content-end order-md-2 order-1">
                                        <div class="invoice-total-wrapper w-100">
                                            ...
                                            <div class="invoice-total-item d-flex justify-content-between" v-if="order?.coupon_discount">
                                                ...
                                                <p class="invoice-total-amount"> - {{ order?.coupon_discount }}</p>
                                            </div>
                                            <div class="invoice-total-item d-flex justify-content-between">
                                                <p class="invoice-total-title">Delivery Charge:</p>
                                                <p class="invoice-total-amount"> + {{ order?.delivery_charge ?? 0 }}</p>
                                            </div>
